-- Anonymous Post Like System Schema
-- Creates tables and RLS policies for anonymous post likes
...
-- Create optional daily events table for operational visibility
...
-- Create indexes
CREATE INDEX IF NOT EXISTS idx_eng_post_like_counters_updated_at 
ON public.eng_post_like_counters (updated_at);

CREATE INDEX IF NOT EXISTS idx_eng_post_like_events_daily_day 
ON public.eng_post_like_events_daily (day);
...
-- RLS Policies for eng_post_like_counters
-- Allow public read access
CREATE POLICY "Allow public read access to post like counters" 
ON public.eng_post_like_counters 
FOR SELECT 
TO anon, authenticated 
USING (true);

-- Only service role can insert/update (no policy needed - service role bypasses RLS)

-- RLS Policies for eng_post_like_events_daily
-- Allow public read access
CREATE POLICY "Allow public read access to daily like events" 
ON public.eng_post_like_events_daily 
FOR SELECT 
TO anon, authenticated 
USING (true);
...
-- Create function to safely increment like count
...
-- Create function to safely decrement like count (floor at 0)
